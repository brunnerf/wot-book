...
curl -X POST "$SERVER/projects" \
     ...
     -d '{ "name": "Wot Example", "description": "First WoT Project" }' > payloads/project.json
...
curl -X POST "$SERVER/projects/$PROJECT_ID/applications" \
     -H "Authorization: $EVRYTHNG_API_KEY" \
     -H "Content-Type: application/json" \
     -d '{ "name": "WoT App", "description": "WoT Client Application","tags":["WoT","device","plug","energy"], "socialNetworks": {} }' > payloads/app.json
